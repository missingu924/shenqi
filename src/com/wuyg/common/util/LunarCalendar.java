...
public class LunarCalendar
{
	...
	final public static String lunarToSolar(String lunarDay)
	{
		try
		{
			if (lunarDay.length() != "2015-11-27".length())
			{
				return "";
			}

			int[] solars = lunarToSolar(StringUtil.parseInt(lunarDay.substring(0, 4)), StringUtil.parseInt(lunarDay.substring(5, 5 + 2)), StringUtil.parseInt(lunarDay.substring(8,
					8 + 2)), false);

			return solars[0] + "-" + (solars[1] < 10 ? "0" + solars[1] : solars[1]) + "-" + (solars[2] < 10 ? "0" + solars[2] : solars[2]);
		} catch (Exception e)
		{
	...
	final public static String solarToLunar(String solorDay)
	{
		try
		{
			Date solorDate = TimeUtil.str2date(solorDay, "yyyy-MM-dd");
			...
			return lunars[0] + "-" + lunars[1] + "-" + lunars[2];
		} catch (Exception e)
		{
			log.error(e.getMessage(), e);
		}
	...
	final public static String solarToLunar(Date solorDate)
	{
		if (solorDate == null)
		{
			return "";
		...
		return lunars[0] + "年" + lunars[1] + "月" + lunars[2] + "日";
	}
	...
	{
		System.out.println(solarToLunar("2015-11-27"));

		System.out.println(lunarToSolar("2015-12-29"));
	}
}
